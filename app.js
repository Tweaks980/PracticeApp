...
export function getDrills() {
  ...
  if (Array.isArray(stored) && stored.length) {
    ...
    const have = new Set(current.map(d => d?.id).filter(Boolean));
    let changed = false;
    for (const d of base) {
...
export function updateShotById(id, patch) {
  if (!id) return false;
  const shots = getShots();
  const idx = shots.findIndex(s => s && s.id === id);
  if (idx === -1) return false;
  const current = shots[idx] || {};
  // Shallow-merge patch; callers should pass full objects for nested fields (e.g., contact).
  shots[idx] = { ...current, ...patch };
  saveJSON(KEYS.SHOTS, shots);
  return true;
}
